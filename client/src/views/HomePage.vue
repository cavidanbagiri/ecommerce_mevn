...
<template>
    <div>
        ...
        <CarouselComp />
        <CategoriesMain />
        <div class="row flex justify-content-space p-1">
                <div v-for="product in products" :key="product?.id" class="card border-none border-bottom mx-2" style="width: 14rem;">
                    ...
                    <div class="card-body">
                        <h5 class="card-title text-md fw-bold">{{product?.brand}}</h5>
                        <p class="card-text text-md py-1">{{product?.name}}</p>
                        <p class="card-text text-md py-1">{{product?.price}} RUB</p>
                        <a href="#" class="btn btn-dark btn-sm">Add Wishlist</a>
                    </div>
                </div>
        </div>
